        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <motion.div
              ...
              onKeyDown={(e) => e.key === 'Enter' && handleCopy('pnpm dlx deployify login', 'login-cmd')}
              role="button"
              tabIndex={0}
              className="mt-12 max-w-md mx-auto p-1 rounded-2xl bg-[var(--card)] border border-[var(--border)] backdrop-blur-sm flex items-center gap-3 pr-4 group cursor-pointer focus-visible:ring-2 focus-visible:ring-[var(--foreground)] focus-visible:outline-none"
            >
              ...
              <div className="relative flex items-center">
                <AnimatePresence mode="wait">
                  <motion.span
                    key={copiedId === 'login-cmd' ? 'copied' : 'copy'}
                    initial={{ opacity: 0, x: 10 }}
                    animate={{ opacity: 1, x: 0 }}
                    exit={{ opacity: 0, x: -10 }}
                    className="absolute right-12 text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted-foreground)] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all"
                  >
                    {copiedId === 'login-cmd' ? 'Copied!' : 'Click to copy'}
                  </motion.span>
                </AnimatePresence>
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  onClick={(e) => { e.stopPropagation(); handleCopy('pnpm dlx deployify login', 'login-cmd'); }}
                  className={cn(
              ...
              >
                {[...['ACME', 'GLOBEX', 'SOYLENT', 'INITECH', 'UMBRELLA', 'CYBERDYNE', 'STARK', 'WAYNE'], ...['ACME', 'GLOBEX', 'SOYLENT', 'INITECH', 'UMBRELLA', 'CYBERDYNE', 'STARK', 'WAYNE']].map((logo, idx) => (
                  <motion.span
                    ...
                    className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--foreground)] opacity-30 grayscale cursor-default transition-all duration-500"
                  >
                    {logo}
                  </motion.span>
                ))}
                ...
                {[
                  ...
                  >
                    <motion.div
                      ...
                      className="absolute -left-12 md:-left-16 -top-2 text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted)]/20 tabular-nums transition-all group-hover:text-[var(--primary)] group-hover:opacity-100"
                      aria-hidden="true"
                    >
                      0{i + 1}
                    </motion.div>
                  ...
                  <motion.div id="repo-results" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} role="listbox" className="absolute top-full left-0 right-0 mt-2 bg-[var(--card)] border border-[var(--border)] rounded-2xl overflow-hidden z-20 shadow-2xl p-2">
                    {filteredRepos.length > 0 ? filteredRepos.map((repo, i) => (
                      <div
                        ...
                        className={cn("px-4 py-2 rounded-xl cursor-pointer flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.2em] transition-colors", selectedIndex === i ? "bg-[var(--muted)]/20 text-[var(--foreground)]" : "text-[var(--muted-foreground)] hover:bg-[var(--muted)]/10 hover:text-[var(--foreground)]")}
                      >
                        <Github className="w-4 h-4" /> {repo}
                      </div>
                    )) : <div className="px-4 py-4 text-center text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted-foreground)]">No results for &quot;{searchQuery}&quot;</div>}
                  </motion.div>
                )}
              </div>
            </div>
