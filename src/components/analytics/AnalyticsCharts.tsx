            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-2 overflow-hidden p-0">
                    <div className="p-6 flex items-center gap-3">
                        ...
                        <div>
                            <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Traffic Trends</span>
                            <h3 className="text-[11px] font-bold">Traffic Over Time</h3>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <Card className="overflow-hidden p-0 h-fit">
                        <div className="p-6 flex items-center gap-3">
                            ...
                            <div>
                                <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Referral Traffic</span>
                                <h3 className="text-[11px] font-bold">Top Sources</h3>
                            </div>
                        </div>
                        <Separator className="bg-[var(--border)]" />
                        <div className="p-6">
                                ...
                                {sources.map((source: { source: string; visitors: number }, index: number) => (
                                    <div key={index} className="flex items-center justify-between group">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[9px] font-bold uppercase tracking-wider">{source.source}</span>
                                        </div>
                                        <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] font-mono">
                                            {source.visitors.toLocaleString()}
                                        </span>
                                    </div>
                                ))}
                                {sources.length === 0 && (
                                    <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">No data available</p>
                                )}
                            </div>
                        </div>
                    </Card>
                    ...
                    <Card className="overflow-hidden p-0 h-fit">
                        <div className="p-6 flex items-center gap-3">
                            ...
                            <div>
                                <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Geographic Reach</span>
                                <h3 className="text-[11px] font-bold">Top Locations</h3>
                            </div>
                        </div>
                        <Separator className="bg-[var(--border)]" />
                        <div className="p-6">
                                ...
                                {locations.map((location: { country: string; visitors: number }, index: number) => (
                                    <div key={index} className="flex items-center justify-between group">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[9px] font-bold uppercase tracking-wider">{location.country}</span>
                                        </div>
                                        <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] font-mono">
        ...
        <Card className="overflow-hidden p-0 hover:border-[var(--primary)] hover:shadow-md transition-all duration-200 group">
            <div className="p-6">
                <div className="flex items-center justify-between mb-2">
                    <span className="text-[9px] font-bold text-[var(--muted-foreground)] uppercase tracking-wider">{title}</span>
                ...
                <div className="flex items-baseline gap-1">
                    <span className="text-[11px] font-bold font-mono tracking-tight group-hover:text-[var(--primary)] transition-colors">
                        {value < 1 ? value.toFixed(3) : Math.round(value)}
                    </span>
                    <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">{unit}</span>
                </div>
                <div className="mt-2">
                    <Badge variant={variant} className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5">
                        {status.replace('-', ' ')}
                    </Badge>
                </div>
                <p className="mt-3 text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] leading-snug line-clamp-2 opacity-60 group-hover:opacity-100 transition-opacity">
                    {description}
                </p>
            </div>
        </Card>
    ...
    if (active && payload && payload.length && label) {
        return (
            <Card className="overflow-hidden p-0 shadow-2xl bg-[var(--card)] border-[var(--border)]">
                <div className="p-3">
                    <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] mb-2">
                        {new Date(label).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase()}
                    </p>
                    <div className="space-y-1">
                        {payload.map((entry: TooltipEntry, index: number) => (
                            <div key={index} className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-wider">
...
function SummaryCard({ title, value }: { title: string; value: string }) {
    return (
        <Card className="overflow-hidden p-0 hover:border-[var(--primary)] hover:shadow-md transition-all duration-300 group">
            <div className="p-6">
                <h3 className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] group-hover:text-[var(--primary)] transition-colors">
                    {title}
                </h3>
                <div className="mt-3 text-[11px] font-bold font-mono tracking-tighter text-[var(--foreground)]">
                    {value}
                </div>
            </div>
        </Card>
