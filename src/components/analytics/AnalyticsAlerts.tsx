...
export function AnalyticsAlerts({ alerts }: AnalyticsAlertsProps) {
    ...
    return (
        <div className="space-y-3">
            <h3 className="text-[11px] font-bold flex items-center gap-2 text-[var(--foreground)]">
            ...
            <div className="space-y-2">
                {alerts.map((alert) => (
                    <div
                        key={alert.id}
                        className={cn(
                            "flex gap-3 p-3 rounded-lg border text-[9px] font-bold uppercase tracking-wider",
                            alert.type === 'critical'
