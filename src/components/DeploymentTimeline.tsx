    ...
    return (
        <div className="w-full py-4">
            <div className="relative flex items-center justify-between">
                ...
                {steps.map((step, index) => {
                    ...
                    return (
                        <div key={step.id} className="flex flex-col items-center gap-2 bg-[var(--background)] px-2 z-10">
                            ...
                            <span className={cn("text-[7px] font-bold uppercase tracking-wider transition-colors",
             ...
             {isError && (
                <div className="mt-4 p-3 rounded-lg bg-[var(--error-bg)] border border-[var(--error)]/20 flex items-start gap-2 text-[9px] text-[var(--error)]">
                    <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
                    <p>{deployment.errorMessage || 'An unknown error occurred during deployment.'}</p>
                </div>
            )}
             {isCancelled && (
                <div className="mt-4 p-3 rounded-lg bg-[var(--muted)]/10 border border-[var(--border)] flex items-start gap-2 text-[7px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">
                    <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
