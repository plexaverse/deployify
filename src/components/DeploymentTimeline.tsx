    ...
    return (
        <div className="w-full py-4">
            <div className="relative flex items-center justify-between">
             ...
             {isError && (
                <div className="mt-4 p-3 rounded-lg bg-[var(--error-bg)] border border-[var(--error)]/20 flex items-start gap-2 text-[11px] text-[var(--error)]">
                    <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
                    <p>{deployment.errorMessage || 'An unknown error occurred during deployment.'}</p>
                </div>
            )}
             {isCancelled && (
                <div className="mt-4 p-3 rounded-lg bg-[var(--muted)]/10 border border-[var(--border)] flex items-start gap-2 text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">
                    <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
