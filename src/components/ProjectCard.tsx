...
const generateSparklineData = (status: string, seed: string) => {
  ...
  return Array.from({ length }, () => ({
    value: Math.max(0, Math.floor(seededRandom(s++) * volatility) + base)
  }));
};
...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <div className={cn("flex flex-col h-full justify-between transition-all duration-500 rounded-2xl bg-[var(--card)]/40 backdrop-blur-sm border border-[var(--border)] hover:border-[var(--foreground)]/20 p-6", config.glow)}>
            ...
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] font-mono bg-[var(--card-hover)]/30 p-2 rounded-lg border border-[var(--border)] group-hover:border-[var(--foreground)]/10 transition-all group/sha">
              ...
              <button
                onClick={(e) => {
                  ...
                  toast.success(`Copied SHA: ${latestDeployment.gitCommitSha.substring(0, 7)}`);
