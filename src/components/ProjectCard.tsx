...
const generateSparklineData = (status: string, seed: string) => {
  ...
  // Seeded random-like generation for visual consistency
  const hash = seed.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);

  return Array.from({ length }, (_, i) => {
    const pseudoRandom = Math.abs(Math.sin(hash + i) * 1000) % volatility;
    return {
      value: Math.max(0, Math.floor(pseudoRandom) + base)
    };
  });
};
