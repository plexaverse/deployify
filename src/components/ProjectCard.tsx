...
const generateSparklineData = (status: string, seed: string) => {
  ...
  return Array.from({ length }, (_, i) => {
    // Deterministic pseudo-random value using Math.sin
    const pseudoRandom = (Math.sin(seedNum + i) + 1) / 2; // 0 to 1
    return {
...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <div className={cn("flex flex-col h-full justify-between transition-all duration-500", config.glow)}>
