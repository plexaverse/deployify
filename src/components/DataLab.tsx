...
// Query: ${query}`;
    ...
    const exportPrismaSchema = () => {
        if (!schema || !schema.columns || !selectedConnector) return;

        let prisma = `// Generated by Deployify Data Lab\n`;
        prisma += `datasource db {\n  provider = "${selectedConnector.type.includes('postgres') ? 'postgresql' : 'mysql'}"\n  url      = env("DATABASE_URL")\n}\n\n`;

        Object.entries(schema.columns).forEach(([table, cols]) => {
            const modelName = table.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join('');
            prisma += `model ${modelName} {\n`;
            cols.forEach(col => {
                let pType = 'String';
                const lowerType = col.type.toLowerCase();
                if (lowerType.includes('int')) pType = 'Int';
                else if (lowerType.includes('bigint')) pType = 'BigInt';
                else if (lowerType.includes('float') || lowerType.includes('decimal') || lowerType.includes('numeric')) pType = 'Float';
                else if (lowerType.includes('bool')) pType = 'Boolean';
                else if (lowerType.includes('timestamp') || lowerType.includes('date')) pType = 'DateTime';
                else if (lowerType === 'json' || lowerType === 'jsonb') pType = 'Json';

                const pk = col.isPrimary ? ' @id' : '';
                const unique = (col.indices?.length || 0) > 0 && !col.isPrimary ? ' @unique' : '';

                prisma += `  ${col.name} ${pType}${pk}${unique}\n`;
            });
            prisma += `  @@map("${table}")\n}\n\n`;
        });
        ...
        const link = document.createElement('a');
        link.setAttribute('href', url);
        link.setAttribute('download', 'schema.prisma');
        link.style.visibility = 'hidden';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        toast.success('Prisma schema exported');
    };

    const exportDrizzleSchema = () => {
        if (!schema || !schema.columns || !selectedConnector) return;

        const isPostgres = selectedConnector.type.includes('postgres');
        let drizzle = `import { ${isPostgres ? 'pgTable, varchar, integer, timestamp, boolean, doublePrecision, jsonb' : 'mysqlTable, varchar, int, timestamp, boolean, double, json'} } from 'drizzle-orm/${isPostgres ? 'pg-core' : 'mysql-core'}';\n\n`;

        Object.entries(schema.columns).forEach(([table, cols]) => {
            const tableName = table.split('_').map((word, i) => i === 0 ? word : word.charAt(0).toUpperCase() + word.slice(1)).join('');
            drizzle += `export const ${tableName} = ${isPostgres ? 'pgTable' : 'mysqlTable'}('${table}', {\n`;
            cols.forEach(col => {
                let dType = `varchar('${col.name}', { length: 255 })`;
                const lowerType = col.type.toLowerCase();

                if (lowerType.includes('int')) dType = isPostgres ? `integer('${col.name}')` : `int('${col.name}')`;
                else if (lowerType.includes('float') || lowerType.includes('decimal') || lowerType.includes('numeric')) dType = isPostgres ? `doublePrecision('${col.name}')` : `double('${col.name}')`;
                else if (lowerType.includes('bool')) dType = `boolean('${col.name}')`;
                else if (lowerType.includes('timestamp') || lowerType.includes('date')) dType = `timestamp('${col.name}')`;
                else if (lowerType === 'json' || lowerType === 'jsonb') dType = isPostgres ? `jsonb('${col.name}')` : `json('${col.name}')`;

                if (col.isPrimary) dType += '.primaryKey()';
                drizzle += `  ${col.name}: ${dType},\n`;
            });
            drizzle += `});\n\n`;
        });

        const blob = new Blob([drizzle], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.setAttribute('href', url);
        link.setAttribute('download', 'schema.ts');
        link.style.visibility = 'hidden';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        toast.success('Drizzle schema exported');
    };
                                                ...
                                                {isExplainResults && col === 'QUERY PLAN' ? (
                                                    <div className="flex flex-col gap-1">
                                                        <div className="flex items-center gap-2">
                                                            {value.includes('Seq Scan') && <span className="text-[10px] px-1 rounded bg-[var(--error)]/20 text-[var(--error)] font-bold">FULL SCAN</span>}
                                                            {value.includes('Index Scan') && <span className="text-[10px] px-1 rounded bg-[var(--success)]/20 text-[var(--success)] font-bold">INDEX</span>}
                                                            {value.includes('Bitmap Index Scan') && <span className="text-[10px] px-1 rounded bg-[var(--success)]/10 text-[var(--success)] font-bold">BITMAP</span>}
                                                            {value.includes('Hash Join') && <span className="text-[10px] px-1 rounded bg-[var(--primary)]/10 text-[var(--primary)] font-bold">HASH JOIN</span>}
                                                            {value.includes('Nested Loop') && <span className="text-[10px] px-1 rounded bg-[var(--warning)]/10 text-[var(--warning)] font-bold">LOOP</span>}
                                                            <span>{value}</span>
                                                        </div>
                                                    </div>
                                                ) : isExplainResults && col === 'type' && value === 'ALL' ? (
                                                    <div className="flex items-center gap-1.5">
                                                        <span className="text-[10px] px-1 rounded bg-[var(--error)]/20 text-[var(--error)] font-bold uppercase">Critical</span>
                                                        {value}
                                                    </div>
                                                ) : (
                                                    value
                                        ...
                                        <div className="mt-4 pt-4 border-t border-[var(--border)] space-y-3 animate-in slide-in-from-top-2">
                                                ...
                                                {(comments[q.id] || []).map(c => (
                                                    <div key={c.id} className="p-2 rounded bg-[var(--muted)]/10 border border-[var(--border)] space-y-1">
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            <span className="text-[10px] font-bold text-[var(--muted-foreground)]/60">{new Date(c.createdAt).toLocaleDateString()}</span>
                                                        </div>
                                                        <p className="text-[10px] font-bold text-[var(--foreground)]">{c.text}</p>
                                                    </div>
                                                ))}
                                            ...
                                            <div className="flex gap-2">
                                                <Input
                                                    ...
                                                    className="h-8 text-[10px] font-bold uppercase tracking-wider placeholder:text-[10px] font-bold"
                                        ...
                                        <div className="flex items-center gap-1 bg-[var(--muted)]/20 p-0.5 rounded-lg border border-[var(--border)]">
                                            {(['all', 'tables', 'collections'] as const).map(t => (
                                                <button
                                                    ...
                                                    className={cn(
                                                        "h-5 px-2 text-[10px] font-bold uppercase tracking-wider rounded-md transition-all",
                                        ...
                                        (() => {
                                            ...
                                            if (items.length === 0) return (
                                                <div className="w-full py-8 text-center border border-dashed border-[var(--border)] rounded-xl opacity-40">
                                                    <span className="text-[10px] font-bold uppercase tracking-wider">No matching entities found</span>
                                                </div>
                                            );
                            ...
                            <div className="flex items-center gap-1 bg-[var(--muted)]/20 p-1 rounded-lg border border-[var(--border)]">
                                ...
                                {(selectedConnector?.type.includes('sql') || ['planetscale', 'alloydb', 'neon', 'cloud-spanner', 'bigquery'].includes(selectedConnector?.type || '')) && (
                                    <Button
                                        ...
                                        className={`h-7 px-3 text-[10px] font-bold uppercase tracking-wider ${viewMode === 'explain' ? 'bg-[var(--background)] shadow-sm text-[var(--primary)]' : 'text-[var(--muted-foreground)]'}`}
                                    >
                        ...
                        {planDrift && (
                            <div className="p-4 rounded-xl bg-[var(--error)]/5 border border-[var(--error)]/20 animate-in slide-in-from-top-2 mb-4">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--error)]">Performance Regression Detected (Plan Drift)</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className={cn(
                                    ...
                                    <p className="text-[10px] font-bold uppercase text-[var(--foreground)]">{planDrift.reason}</p>
                                </div>
                            </div>
                        )}
                        ...
                        {optimizationSuggestions && (
                            <div className="p-4 rounded-xl bg-[var(--primary)]/5 border border-[var(--primary)]/20 animate-in slide-in-from-top-2 mb-4">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-bold uppercase text-[var(--muted-foreground)]">Avg Impact Score:</span>
                                ...
                                <div className="space-y-2">
                                    {optimizationSuggestions.sort((a, b) => b.score - a.score).map((s, i) => (
                                        <div key={i} className={cn(
                                            "flex items-start gap-3 text-[10px] font-bold uppercase tracking-wider p-2 rounded border transition-all",
                                            ...
                                            <div className="space-y-1 flex-1">
                                                <div className="flex items-center gap-2">
                                                    <span className="px-1 py-0.5 rounded bg-current/10 text-[10px] font-bold">{s.severity.toUpperCase()}</span>
                                            ...
                                            {s.action?.type === 'apply_index' && (
                                                <Button
                                                    ...
                                                    className="h-7 px-2 text-[10px] font-bold uppercase tracking-wider text-[var(--primary)] border border-[var(--primary)]/20 hover:bg-[var(--primary)]/10 shrink-0"
                                                >
            ...
            {viewingAuditQuery && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--background)]/80 backdrop-blur-sm p-4">
                    <Card className="w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="p-6">
                            <div className="p-4 bg-[var(--card)] border border-[var(--border)] rounded-xl font-mono text-[10px] font-bold max-h-96 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="p-6 space-y-4">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    className="placeholder:text-[10px] font-bold"
                                    autoFocus
                                />
                            </div>
