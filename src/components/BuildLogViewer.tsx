...
export function BuildLogViewer({ logs, loading, error, onRetry, className }: BuildLogViewerProps) {
        ...
        return (
            <div className={cn("flex flex-col items-center justify-center h-full text-[var(--muted-foreground)] bg-[var(--terminal-bg)] p-8 gap-4", className)}>
                ...
                <div className="space-y-1 text-center">
                   <p className="text-[9px] font-bold uppercase tracking-wider">Initialization</p>
                   <p className="text-[11px] font-bold">Waiting for build logs...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div
            ref={containerRef}
            className={cn("h-full overflow-y-auto p-6 font-mono text-[9px] leading-relaxed bg-[var(--terminal-bg)] selection:bg-[var(--primary)]/30", className)}
        >
            <div className="flex flex-col space-y-0.5 text-[var(--terminal-foreground)]/80">
                {logs?.split('\n').map((line, i) => (
                    <div key={i} className="break-all whitespace-pre-wrap hover:bg-[var(--terminal-foreground)]/5 transition-colors duration-200 px-2 rounded-sm -mx-2 group flex gap-4">
                        <span className="shrink-0 text-[9px] font-bold uppercase tracking-wider opacity-30 select-none w-10 text-right">
            ...
            {logs && !error && !loading && (
                <div className="flex items-center gap-2 mt-8 py-3 px-4 rounded-lg bg-[var(--success)]/5 border border-[var(--success)]/10 text-[var(--success)]">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-wider">Build process completed successfully</span>
                </div>
            )}
            ...
            {error && (
                <div className="flex flex-col items-start mt-8 p-6 rounded-xl bg-[var(--error)]/5 border border-[var(--error)]/10 gap-4">
                    <div className="flex items-center gap-2 text-[var(--error)]">
                        ...
                        <span className="text-[9px] font-bold uppercase tracking-wider">Deployment Pipeline Error</span>
                    </div>
                    <p className="text-[11px] font-bold text-[var(--error)]/90 break-all whitespace-pre-wrap">{error}</p>
                    {onRetry && (
                        <Button
                            onClick={onRetry}
                            variant="secondary"
                            size="sm"
                            className="text-[9px] font-bold uppercase tracking-wider h-8 px-4"
                        >
