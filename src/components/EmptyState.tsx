...
export function EmptyState({
    ...
    return (
        <div className={cn("flex flex-col items-center justify-center py-16 px-6 border-2 border-dashed border-[var(--border)] rounded-3xl bg-[var(--muted)]/5 text-center animate-fade-in shadow-sm", className)}>
            ...
            <h3 className="text-[11px] font-bold mb-2 tracking-tight text-[var(--foreground)]">{title}</h3>
            <div className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] max-w-sm mb-8 leading-relaxed">
                {description}
            </div>
            {children}
            {action && (
