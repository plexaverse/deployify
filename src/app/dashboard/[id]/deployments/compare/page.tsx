...
export default function CompareDeploymentsPage() {
    ...
    const renderMetricDiff = (
        ...
        if (isNeutral) return <span className="text-[var(--muted-foreground)] text-[9px] font-bold uppercase tracking-wider ml-2">No change</span>;

        return (
            <span className={cn("text-[9px] font-bold uppercase tracking-wider ml-2 flex items-center gap-0.5", isImprovement ? "text-[var(--success)]" : "text-[var(--error)]")}>
    ...
    return (
        <div className="max-w-7xl mx-auto px-6 md:px-8 py-8 space-y-10">
            ...
            <Button
                ...
                size="sm"
                onClick={() => router.back()}
                className="group text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] hover:text-[var(--foreground)] -ml-2 h-auto py-0"
            >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                        <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Performance Analysis</span>
                        <h1 className="text-[9px] md:text-[11px] font-bold tracking-tight">Compare Deployments</h1>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <Card className="overflow-hidden p-0">
                    <div className="p-6 flex items-center gap-3">
                        ...
                        <div>
                            <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Base Deployment</span>
                            <h3 className="text-[11px] font-bold">Previous Version</h3>
                        </div>
                    </div>
                    <Separator className="bg-[var(--border)]" />
                    <div className="p-6 space-y-6">
                        <div>
                            <label className="text-[11px] font-bold mb-3 block">Select Source</label>
                            <NativeSelect
                                value={baseId}
                        ...
                        {baseDeployment ? (
                            <DeploymentSummary deployment={baseDeployment} />
                             ...
                             <div className="h-48 flex items-center justify-center text-[var(--muted-foreground)] border border-dashed border-[var(--border)] rounded-lg text-[11px] font-bold">
                                Select a deployment
                            </div>
                        )}
                    </div>
                ...
                <Card className="overflow-hidden p-0">
                    <div className="p-6 flex items-center gap-3">
                        ...
                        <div>
                            <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Target Deployment</span>
                            <h3 className="text-[11px] font-bold">New Version</h3>
                        </div>
                    </div>
                    <Separator className="bg-[var(--border)]" />
                    <div className="p-6 space-y-6">
                        <div>
                            <label className="text-[11px] font-bold mb-3 block">Select Target</label>
                            <NativeSelect
                                value={targetId}
            ...
            {baseDeployment && targetDeployment && (
                <Card className="overflow-hidden p-0 border-[var(--border)] shadow-sm">
                    <div className="px-6 py-4 border-b border-[var(--border)] flex items-center gap-3">
                        ...
                        <div>
                            <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Performance Metrics</span>
                            <h3 className="text-[11px] font-bold">Comparison Results</h3>
                        </div>
                    </div>
        ...
        <div className="space-y-6">
            <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                    <h3 className="text-[11px] font-bold text-[var(--foreground)] truncate">{deployment.gitCommitMessage}</h3>
                    <div className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] mt-1">
                ...
                <Badge variant={deployment.status === 'ready' ? 'success' : 'secondary'} className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-[var(--muted)]/5 rounded-xl border border-[var(--border)]/50 transition-colors hover:bg-[var(--muted)]/10">
                    <div className="text-[var(--muted-foreground)] text-[9px] font-bold uppercase tracking-wider mb-1">Created</div>
                    <div className="text-[11px] font-bold text-[var(--foreground)]">{new Date(deployment.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase()}</div>
                </div>
                 <div className="p-4 bg-[var(--muted)]/5 rounded-xl border border-[var(--border)]/50 transition-colors hover:bg-[var(--muted)]/10">
                    <div className="text-[var(--muted-foreground)] text-[9px] font-bold uppercase tracking-wider mb-1">Environment</div>
                    <div className="text-[11px] font-bold text-[var(--foreground)] uppercase">{deployment.type}</div>
                </div>
            </div>
        </div>
    );
...
function ComparisonRow({ label, baseValue, targetValue }: { label: string, baseValue: React.ReactNode, targetValue: React.ReactNode }) {
    return (
        <div className="grid grid-cols-3 px-6 py-4 items-center hover:bg-[var(--card-hover)] transition-colors group">
            <div className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">{label}</div>
            <div className="text-[11px] font-mono font-bold">{baseValue}</div>
