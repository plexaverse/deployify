...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-8 py-8 space-y-10">
                  ...
                  infraHealth && (
                    <>
                      <div className="text-right">
                        <span className="block text-[8px] font-bold uppercase text-[var(--muted-foreground)]">
                          Global Uptime
                        </span>
                        <span
                          className={cn(
                            "text-[10px] font-bold uppercase",
                            infraHealth.summary.uptimeScore > 98
                              ? "text-[var(--success)]"
                              : "text-[var(--warning)]",
                          )}
                        >
                          {infraHealth.summary.uptimeScore}%
                        </span>
                      </div>
                      <Separator
                        orientation="vertical"
                        className="h-8 bg-[var(--border)]"
                      />
                      <div className="flex items-center gap-3">
                        <div className="text-center">
                          ...
                          <span className="text-[10px] font-bold text-[var(--success)]">
                            {infraHealth.summary.healthyConnectors}
                          </span>
                        </div>
                        <div className="text-center">
                          <span className="block text-[8px] font-bold uppercase text-[var(--muted-foreground)]">
                          ...
                          <span className="text-[10px] font-bold text-[var(--warning)]">
                            {infraHealth.summary.degradedConnectors}
                          </span>
                        </div>
                        <div className="text-center">
                          <span className="block text-[8px] font-bold uppercase text-[var(--muted-foreground)]">
                          ...
                          <span className="text-[10px] font-bold text-[var(--error)]">
                            {infraHealth.summary.unhealthyConnectors}
                          </span>
                        </div>
                      </div>
                    </>
          ...
          <Card className="overflow-hidden p-4 border-[var(--primary)]/10 bg-[var(--primary)]/5 flex flex-col justify-center">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-xl font-bold tracking-tighter">
                {loadingInfra
                  ? "..."
                  : infraHealth?.summary.totalConnectors || 0}
              </span>
      ...
      {!loading && projects.length === 0 && <OnboardingGuide />}
          ...
          >
            <BentoGrid>
              {filteredProjects.map((project) => {
                ...
                const glowClass =
                  {
                    ready: "hover:shadow-[0_0_30px_var(--success-bg)]",
                    building: "hover:shadow-[0_0_30px_var(--warning-bg)]",
                    deploying: "hover:shadow-[0_0_30px_var(--info-bg)]",
                    error: "hover:shadow-[0_0_30px_var(--error-bg)]",
                    queued: "hover:shadow-xl",
                    cancelled: "hover:shadow-xl",
                  }[status as string] || "hover:shadow-xl";
                ...
                return (
                  ...
                  >
                    <BentoGridItem
                      ...
                      className={cn(
                        "h-full min-h-[12rem] cursor-pointer p-0 transition-all duration-500",
                        glowClass,
                      )}
                    />
                  </Link>
