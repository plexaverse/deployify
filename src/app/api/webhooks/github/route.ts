...
async function handlePullRequestEvent(payload: GitHubPullRequestEvent): Promise<void> {
                ...
                for (const storage of project.storageConfigs) {
                    ...
                    if (storage.type === 'neon') {
                        ...
                        if (neonProjectId && providerApiKey) {
                            ...
                            try {
                                // 1. List branches to find ID by name
                                const listRes = await fetch(`https://console.neon.tech/api/v2/projects/${neonProjectId}/branches`, {
                                    headers: { 'Authorization': `Bearer ${providerApiKey}` }
                                });
                                if (listRes.ok) {
                                    const data = await listRes.json();
                                    const branch = data.branches?.find((b: { name: string, id: string }) => b.name === identifier);
                                    if (branch) {
                                        // 2. Delete branch by ID
    ...
    if (action === 'opened' || action === 'synchronize' || action === 'reopened') {
        ...
        try {
        // Orchestrate Database Branching if enabled
        if (project.storageConfigs && project.storageConfigs.length > 0) {
            for (const storage of project.storageConfigs) {
                if (storage.branchingSettings?.enabled && storage.type.includes('cloud-sql')) {
                    ...
                    if (instanceName) {
                        try {
                            ...
                            console.log(`[Branching] Pre-provisioning database ${dbName} for PR #${pull_request.number}`);
