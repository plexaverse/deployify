    ...
    if (isLoading) {
        return (
            <div className="min-h-screen bg-[var(--background)] flex flex-col items-center justify-center p-6 space-y-4">
                ...
                <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Fetching shared insight...</span>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-[var(--background)] flex flex-col items-center justify-center p-6 text-center space-y-4">
                ...
                <div className="space-y-2">
                    <h2 className="text-[9px] md:text-[11px] font-bold uppercase">Access Denied</h2>
                    <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)] max-w-md mx-auto">
        ...
        <div className="min-h-screen bg-[var(--background)] p-6 md:p-8">
            <div className="max-w-6xl mx-auto space-y-8">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--primary)]">Shared Insight</span>
                            ...
                            <h1 className="text-[9px] md:text-[11px] font-bold tracking-tight uppercase">{widget?.name}</h1>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-8 flex flex-col">
                        {isExecuting && !results && (
                            <div className="flex-1 flex flex-col items-center justify-center space-y-4">
                                ...
                                <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Running secure query...</span>
                            </div>
                        )}
                    ...
                    <div className="p-4 bg-[var(--muted)]/5 border-t border-[var(--border)] flex items-center justify-between">
                        <div className="flex items-center gap-4 text-[var(--muted-foreground)]">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[9px] font-bold uppercase tracking-wider">Secure Managed Storage</span>
                            </div>
                            <Separator orientation="vertical" className="h-4 bg-[var(--border)]" />
                            <span className="text-[9px] font-bold uppercase tracking-wider">{results?.length || 0} Rows Returned</span>
                        </div>
                        <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]/50">
                ...
                <div className="text-center">
                    <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]/40 max-w-xl mx-auto">
            ...
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-[var(--muted)]/20 border-b border-[var(--border)]">
                        {columns.map(col => (
                            <th key={col} className="p-4 text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">
                                {col}
                            </th>
                        ))}
                    </tr>
                ...
                <tbody>
                    {results.map((row, i) => (
                        <tr key={i} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--muted)]/5 transition-colors">
                            {columns.map(col => (
                                <td key={col} className="p-4 text-[9px] font-mono whitespace-nowrap max-w-[300px] truncate">
