        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                    <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Developer Tools</span>
                    <h1 className="text-[9px] md:text-[11px] font-bold tracking-tight">Edge Function Simulator</h1>
                </div>
            </div>
            <MovingBorderButton
                onClick={handleRun}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[calc(100vh-200px)]">
            ...
            <Card className="overflow-hidden p-0 flex flex-col h-full">
                <div className="p-6 flex items-center gap-3">
                    ...
                    <div>
                        <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Code Editor</span>
                        <h2 className="text-[9px] md:text-[11px] font-bold">Middleware Code</h2>
                    </div>
                </div>
                ...
                <div className="flex-1 p-0 overflow-hidden bg-[var(--terminal-bg)]">
                     <textarea
                        ...
                        className="w-full h-full bg-transparent text-[var(--terminal-foreground)] font-mono text-[11px] p-4 focus:outline-none resize-none"
                        spellCheck={false}
                    />
                </div>
            </Card>
                ...
                <Card className="overflow-hidden p-0">
                    <div className="p-6 flex items-center gap-3">
                        ...
                        <div className="space-y-0.5">
                            ...
                            <h2 className="text-[9px] md:text-[11px] font-bold text-[var(--foreground)]">Request Configuration</h2>
                        </div>
                    </div>
                    <Separator className="bg-[var(--border)]" />
                    ...
                    <div className="space-y-2">
                        <Label className="text-[11px] font-bold">HTTP Method</Label>
                        <SegmentedControl
                            options={[
                                { value: 'GET', label: 'GET' },
                    ...
                    <div className="space-y-2">
                        <Label className="text-[11px] font-bold mb-2 block">Headers (JSON)</Label>
                        <textarea
                            value={headers}
                            onChange={(e) => setHeaders(e.target.value)}
                            className={cn(
                                "flex min-h-[100px] w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-[11px]",
                 ...
                 <div className="space-y-4 flex-1 flex flex-col">
                    <Card className="overflow-hidden p-0 flex flex-col flex-1">
                        <div className="p-6 flex items-center gap-3">
                            ...
                            <div>
                                <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Execution Output</span>
                                <h2 className="text-[9px] md:text-[11px] font-bold">Simulation Result</h2>
                            </div>
                        </div>
                        <Separator className="bg-[var(--border)]" />
                        ...
                        <div className="p-6 flex-1 flex flex-col">
                            {!result && !loading && (
                                <div className="flex-1 border border-dashed border-[var(--border)] rounded-xl flex items-center justify-center text-[var(--muted-foreground)] p-8 min-h-[200px] text-[9px] font-bold uppercase tracking-wider">
                            ...
                            {result && (
                                <div className="flex flex-col gap-6">
                                    ...
                                    {result.logs.length > 0 && (
                                        <div className="space-y-2">
                                            <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Runtime Logs</span>
                                            <div className="p-4 rounded-xl border border-[var(--terminal-border)] bg-[var(--terminal-bg)] text-[var(--terminal-foreground)] font-mono text-[9px] font-bold uppercase tracking-wider overflow-auto max-h-[150px]">
                                    ...
                                    {result.error && (
                                        <div className="space-y-2">
                                            <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--error)]">Error Details</span>
                                            <div className="p-4 text-[var(--error)] bg-[var(--error-bg)] border border-[var(--error)]/20 rounded-xl font-mono text-[9px] font-bold uppercase tracking-wider">
                                                {result.error}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {!result.error && (
                                        <div className="space-y-6">
                                            ...
                                            {Object.keys(result.headers).length > 0 && (
                                                <div className="space-y-2">
                                                    <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Response Headers</span>
                                                    <div className="bg-[var(--muted)]/5 p-4 rounded-xl font-mono text-[9px] font-bold uppercase tracking-wider overflow-auto border border-[var(--border)]">
                                            ...
                                            {/* Body */}
                                            <div className="space-y-2">
                                                <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--muted-foreground)]">Response Body</span>
                                                <pre className="bg-[var(--muted)]/5 p-4 rounded-xl font-mono text-[9px] font-bold uppercase tracking-wider overflow-auto max-h-[300px] whitespace-pre-wrap border border-[var(--border)] text-[var(--foreground)]">
