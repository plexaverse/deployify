...
/**
 * Perform data anonymization during seeding or cloning (Phase 122 - SQL Masking)
 */
export async function anonymizeData(
    ...
    if (process.env.MOCK_DB === 'true') {
        console.log(`[Anonymizer] MOCK: Anonymizing ${isPostgres ? 'Postgres' : 'MySQL'} data...`);
        return;
    }

    const salt = process.env.ANONYMIZATION_SALT || 'deployify-salt';
    ...
    if (isPostgres) {
        ...
        try {
            for (const config of tableConfig) {
                const setClause = config.columns
                    .map(col => `\`${col}\` = MD5(CONCAT(\`${col}\`, '${salt}'))`)
                    .join(', ');
    ...
    console.log(`[Anonymizer] Data masking completed for ${tableConfig.length} tables.`);
}
