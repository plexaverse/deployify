...
export async function ensureEphemeralDatabase(
    ...
    // 2. Create the target ephemeral database
    const createOp = await createDatabase(instanceName, databaseName);
    await waitForOperation(createOp);

    // 3. If source database is provided, seed the ephemeral database via export/import
    if (sourceDatabase) {
        ...
        try {
            ...
            // We still proceed even if seeding fails, as the empty DB is better than nothing for some tests
        }
    }
}
