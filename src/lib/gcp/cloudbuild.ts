            ...
            args: [
                ...
                `PROJECT_NUMBER=$(gcloud projects describe ${gcpProjectId} --format="value(projectNumber)") && \
                 ...
                 ${cloudSqlInstances.length > 0 ? `gcloud projects add-iam-policy-binding ${gcpProjectId} --member="serviceAccount:$\${RUN_SERVICE_AGENT}" --role="roles/cloudsql.client" --quiet || echo "Warning: Could not grant Cloud SQL client role to $\${RUN_SERVICE_AGENT}"; \
                   gcloud projects add-iam-policy-binding ${gcpProjectId} --member="serviceAccount:$\${RUN_SERVICE_AGENT}" --role="roles/cloudsql.instanceUser" --quiet || echo "Warning: Could not grant Cloud SQL instance user role to $\${RUN_SERVICE_AGENT}"; \
                 gcloud projects add-iam-policy-binding ${gcpProjectId} --member="serviceAccount:$\${COMPUTE_SA}" --role="roles/cloudsql.client" --quiet || echo "Warning: Could not grant Cloud SQL client role to $\${COMPUTE_SA}"; \
                   gcloud projects add-iam-policy-binding ${gcpProjectId} --member="serviceAccount:$\${COMPUTE_SA}" --role="roles/cloudsql.instanceUser" --quiet || echo "Warning: Could not grant Cloud SQL instance user role to $\${COMPUTE_SA}"` : 'echo "No Cloud SQL instances to bind"'}`,
            ],
        }] : []),
