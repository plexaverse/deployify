...
## 2026-03-24 - [Tactile Grid Stability and Layout Tidiness]
**Learning:** Transitioning from horizontal translations to a vertical lift (`whileHover={{ y: -4 }}`) for grid items provides a more premium feel without causing horizontal layout shifts for adjacent elements. Additionally, when a grid item wraps a complex card that already manages its own borders and padding, stripping the wrapper's default styles (via `p-0 border-0`) prevents "double-border" visual clutter and maintains a clean, scripe.io-like aesthetic.
**Action:** Use vertical lifts for grid hovers and eliminate redundant wrapper styles when nesting interactive cards.
