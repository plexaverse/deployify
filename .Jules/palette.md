...
## 2026-03-25 - [Grid Containment and Tactile Responsiveness]
**Learning:** In interactive dashboard grids, nesting cards with their own borders inside grid items that also have borders creates visual "noise" and redundant padding. Outsourcing the containment and glass-morphism styles to the child component while stripping the grid item of its default borders/padding creates a cleaner, more integrated look. Additionally, adding `whileTap` scaling to grid items provides essential tactile feedback that makes the UI feel "physical" and premium.
**Action:** Apply `p-0 border-0 bg-transparent shadow-none` to `BentoGridItem` when it wraps a `ProjectCard`, and ensure all interactive grid elements include `whileTap` transitions.
