...
try {
    ...
    // We expect COPY commands to reference apps/web
    if (!nextDockerfile.includes('COPY --from=builder /app/apps/web/public ./apps/web/public')) {
    ...
    if (!nextDockerfile.includes('COPY --from=builder --chown=nextjs:nodejs /app/apps/web/.next/static ./apps/web/.next/static')) {
        console.error('Next.js Dockerfile missing correct static copy');
        console.log(nextDockerfile);
        process.exit(1);
    }
    // CMD check
    if (!nextDockerfile.includes('CMD ["node", "apps/web/server.js"]')) {
